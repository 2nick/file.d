...
func TestDecode(t *testing.T) {
	config := &Config{
		Field:  "log",
		Prefix: "prefix.",
		Re2:    "(?P<date>[\\d]{4}-[\\d]{2}-[\\d]{2} [\\d]{2}:[\\d]{2}:[\\d]{2} GMT) \\[(?P<pid>[\\d]+)\\] => \\[(?P<pid_message_number>[\\d-]+)\\] client=(?P<client>[^,]*),db=(?P<db>[^,]*),user=(?P<user>[^,]*) LOG:  (?P<message>.+)",
	}
	...
	assert.Equal(t, `{"date":"2021-06-22 16:24:27 GMT","pid":"7291","pid_message_number":"2-1","client":"test_client","db":"test_db","user":"test_user","message":"listening on IPv4 address \"0.0.0.0\", port 5432"}`, outEvents[0].Root.EncodeToString(), "wrong out event")
}
