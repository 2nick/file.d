...
func (p *Plugin) getNewLimiter(limiterKey string, rule *rule) limiter {
	...
	default:
		panic(fmt.Sprintf("Unknown limiter kind '%s' - expecting 'redis' or 'memory'", p.config.LimiterKind))
	}
}
...
func (p *Plugin) isAllowed(event *pipeline.Event) bool {
	...
	for index, rule := range p.rules {
		...
		if !has {
			...
			if !has {
				limiter = p.getNewLimiter(limiterKey, rule)
